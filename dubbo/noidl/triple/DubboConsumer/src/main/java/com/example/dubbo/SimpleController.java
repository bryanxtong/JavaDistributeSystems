...
public class SimpleController {
    ...
    public Mono<String> sayHello(@RequestParam("name") String name) {
        return Mono.fromCallable(() -> simpleService.sayHello(new HelloRequest(name)).getMessage());
    }
    ...
    public Flux<String> streamHello(@RequestParam("name") String name) {
        return Flux.create(emitter -> {
            simpleService.streamHello(new HelloRequest(name), new StreamObserver<>() {
                @Override
    ...
    public Mono<String> clientStreamHello(@RequestBody List<String> names) {
        return Mono.create(emitter -> {
            ...
            names.forEach(name -> requestObserver.onNext(new HelloRequest(name)));
    ...
    public Flux<String> biStreamHello(@RequestBody Flux<String> names) {
        return Flux.create(emitter -> {
            ...
            names.subscribe((String name) -> requestObserver.onNext(new HelloRequest(name)),

...
public class SpringCloudNameResolver extends NameResolver{
    private static final String GRPC_PORT = "grpc.port"; //grpc.port in instance metadata
    ...
    private void resolve() {
        ...
        //System.out.println("Instances Started: " + instances);
        Map<String, List<InetSocketAddress>> addrStore = ImmutableMap.<String, List<InetSocketAddress>>builder().put(StubConfiguration.serviceName, instances
                        .stream()
                        .map(serviceInstance -> new InetSocketAddress(serviceInstance.getHost(), Integer.valueOf(serviceInstance.getMetadata().getOrDefault(GRPC_PORT, "0"))))
